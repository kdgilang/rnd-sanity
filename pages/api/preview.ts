...
import { getSecret } from '@sanity/lib/utils'
...
function redirectToPreview(
  ...
  previewData: { token?: string, status: boolean },
  Location: string
): void {
...
export default async function preview(
  ...
  const previewData: { token?: string, status: boolean } = {
    status: true
  }
  ...
  return redirectToPreview(res, previewData, redirect)
}